...
public:
  ...
  void enqueue_eedomus(int p_id, float val, bool is_f, bool force = false) {
    if (!eedomus_enabled_.load()) return;
    ...
    if (eedomus_queue_.size() > 25) eedomus_queue_.pop_back();
  ...
  void run_watchdog() {
    uint32_t now = millis();
    if (link_sensor_) link_sensor_->publish_state(now - last_spa_ms_.load() < 3500);
    if (kbd_link_sensor_) kbd_link_sensor_->publish_state(now - last_kbd_ms_.load() < 3500);
  ...
  void loop() override {
    ...
    if (http_busy_ && (now - last_http_start_ms_ > 10000)) {
      ESP_LOGW(TAG, "Fail-Safe: Libération forcée du verrou HTTP (Timeout 10s)");
      http_busy_ = false;
    }
    ...
    if (eedomus_enabled_.load() && !http_busy_.load() && !eedomus_queue_.empty() && (now - last_http_ms_ > 5000)) {
...
protected:
  ...
  void uart_task() {
    ...
    if (is_sync_.load()) {
        if (retry_f_.load() > 0) { if(inject_cmd(0x01, target_f_.load() ? 0x01 : 0x00)) retry_f_--; }
        else if (retry_h_.load() > 0) { if(inject_cmd(0x01, target_h_.load() ? 0x01 : 0x00)) retry_h_--; }
        else if (retry_u_.load() > 0) { if(inject_cmd(0x01, target_u_.load() ? 0x01 : 0x00)) retry_u_--; }
        else if (retry_b_.load() > 0) { if(inject_cmd(0x03, (uint8_t)target_b_.load())) retry_b_--; }
    }
  ...
  void process_machine(uint8_t c, bool from_spa) {
    ...
    if (*idx_ptr > 0) {
        ...
        if (now - last > 100) *idx_ptr = 0;
    }
    
    if (from_spa) last_spa_ms_.store(now); else last_kbd_ms_.store(now);

    if (*idx_ptr == 0) { 
        if (c == 0xA5 || c == 0x00) { buf[0] = c; *idx_ptr = 1; } 
        return; 
    }
      ...
      if (*idx_ptr == len) {
        ...
        if (valid) {
          if (!from_spa && (!lock_kbd_.load() || buf[1] == 0x0D)) {
            // Relais Clavier -> SPA (Mutex nécessaire pour ne pas entrer en collision avec Sniper)
  ...
  void handle_frame(uint8_t id, uint8_t d1, uint8_t d2, bool from_spa) {
    ...
    if (id == 0x06 || id == 0x1B || id == 0x1A) {
      ...
      if (target_b_.load() != bus_b_.load() && retry_b_.load() > 0) { if (inject_cmd(0x03, (uint8_t)target_b_.load())) retry_b_--; }
    }
  }
