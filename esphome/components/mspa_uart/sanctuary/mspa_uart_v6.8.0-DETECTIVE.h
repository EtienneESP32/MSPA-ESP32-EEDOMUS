 ...
 public:
  ...
  void loop() override {
    ...
    if (millis() - last_scan > 2000) {
      last_scan = millis();
      // ESP_LOGV("detective", "Scan..."); // Verbose only to not clutter
      this->scan_sockets();
    }
    ...
    for (int i = 0; i < 64; i++) {
      ...
      if (getpeername(i, (struct sockaddr *)&addr, &addr_len) == 0) {
        if (addr.sin_family == AF_INET) {
           ESP_LOGI("detective", "Socket FD=%d connected to %s", i, inet_ntoa(addr.sin_addr));
           found++;
        }
      }
    }
    // if (found > 0) ESP_LOGD("detective", "Found %d active sockets", found);
  }
